...
class Solution:
    def TOH(self,num ,src,hel,des):
        if num==1:
            print(f"Move 1 from {src} to {des}")
            return 

        self.TOH(num-1,src,des,hel)
        print(f"Move {num} from {src} to {des}")
        self.TOH(num-1,hel,src,des)

def main():
    num = int(input("Enter the no of piles:"))
    print(2**num-1)
    sol = Solution()
    sol.TOH(num,"A","B","C")
